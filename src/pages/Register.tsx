...
const Register: React.FC = () => {
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    if (result.error) { setError(result.error); setLoading(false); }
      ...
      setSuccess(true);
      setLoading(false);
      // Auto-confirm is enabled, redirect to chat after short delay
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-sm text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">Akun Berhasil Dibuat! 🎉</h1>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="w-full max-w-sm relative z-10">
        ...
        <button onClick={handleGoogleRegister} disabled={loading} className="w-full py-3 rounded-xl bg-card border border-border text-foreground font-medium text-sm hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2.5">
