...
const StudyOnboarding: React.FC = () => {
  ...
  useEffect(() => {
    const existing = localStorage.getItem("aqua-study-profile");
    if (existing) {
      try {
        const parsed = JSON.parse(existing);
        if (parsed.age && parsed.education) {
        ...
        <AnimatePresence mode="wait">
          {step === 0 && (
            <motion.div key="welcome" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="text-center">
              ...
              <p className="text-muted-foreground text-sm leading-relaxed mb-8 max-w-sm mx-auto">
                AI akan mengajarkanmu semuanya yang kamu inginkan. Sebelum itu, ceritakan sedikit tentang dirimu agar materi disesuaikan untukmu.
              </p>
          ...
          {step === 1 && (
            <motion.div key="age" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="text-center">
              ...
              <p className="text-muted-foreground text-sm mb-8">Kami akan menyesuaikan konten pembelajaran untukmu</p>
          ...
          {step === 2 && (
            <motion.div key="education" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="text-center">
              ...
              <p className="text-muted-foreground text-sm mb-8">Pilih jenjang pendidikanmu saat ini</p>
